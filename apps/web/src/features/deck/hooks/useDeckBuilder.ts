...
export interface UseDeckBuilderReturn {
  ...
  newDeck: () => void;
  /** Save current deck to localStorage. Returns false if storage is full. */
  saveDeck: () => boolean;
...
export function useDeckBuilder(): UseDeckBuilderReturn {
  ...
  const saveDeck = useCallback((): boolean => {
    try {
      const currentDeck = deckRef.current;
      ...
      for (const candidate of candidates) {
        ...
        for (const { card: deckCard } of deck.cards) {
          ...
          if (hasSynergy) {
            ...
            // Weight by strength from both directions
            for (const syn of forward.synergies) {
              totalStrength += strengthWeight(syn.strength);
            }
            for (const syn of reverse.synergies) {
              totalStrength += strengthWeight(syn.strength);
            }
          }
        }
