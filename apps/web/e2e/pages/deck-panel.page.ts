...
export class DeckPanelPage {
  ...
  async incrementCard(cardName: string) {
    const addButton = this.page.getByLabel(new RegExp(`Add one copy of ${cardName}`, 'i'));
    await addButton.click();
  }

  async decrementCard(cardName: string) {
    const removeButton = this.page.getByLabel(new RegExp(`Remove one copy of ${cardName}`, 'i'));
