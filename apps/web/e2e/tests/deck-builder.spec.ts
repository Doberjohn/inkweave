...
test.describe('Deck Builder', () => {
  ...
  test('should remove card from deck', async ({cardListPage, deckPanelPage}) => {
    // Add a card first
    await cardListPage.filterByInk('Amber');
    await cardListPage.addFirstCardToDeck();
  ...
  test('should increment card quantity in deck', async ({cardListPage, deckPanelPage}) => {
    // Add a card
  ...
  test('should not exceed 4 copies of a card', async ({cardListPage, deckPanelPage, page}) => {
    ...
    const addButton = page.getByLabel(new RegExp(`Add one copy of ${cardName}`, 'i'));
    await expect(addButton).toBeHidden();
  });

  test('should show deck card count badge', async ({cardListPage, deckPanelPage}) => {
  ...
  test('should warn about 3+ ink colors', async ({cardListPage, deckPanelPage, page}) => {
